...
def task1(device):
    ...
    tensorboard_log_path = os.path.join('tb_log', 'task1')
    checkpoint_path = os.path.join('checkpoint', 'task1')
    os.makedirs(tensorboard_log_path, exist_ok=True)
    os.makedirs(checkpoint_path, exist_ok=True)

    writer = SummaryWriter(tensorboard_log_path)

    task1_train_dataset = Task1TrainDataset('./data/task1_dataset_kotae.csv')
    task1_train_dataloader = DataLoader(task1_train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    ...
    for epoch_id in range(num_epochs):
        for batch_id, batch in enumerate(tqdm(task1_train_dataloader)):
            ...
            step = epoch_id * len(task1_train_dataloader) + batch_id
